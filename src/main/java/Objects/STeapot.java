...
public class STeapot extends SObject {
    ...
    protected void genData() {
        ...
        for (k = 0; k < 10; k++) {
            for (i = 0; i < numPointsPerEdge; i++)
                for (j = 0; j < numPointsPerEdge; j++) {
                    ...
                    v0.normalize();
                    textures[2 * m + 1] = (float) (Math.acos(v0.getZ()) / Math.PI);
                    textures[2 * m] = (float) (0.5 + 0.5 * Math.atan2(v0.getY(), v0.getX()) / Math.PI);
                    // modify texture coordinates to remedy
                    // the texture holes
                    if (Math.abs(textures[2 * m]) > 1 - EPS)
                        textures[2 * m] = 0;
                    // y-axis reflection
                    textures[20 * numPointsPerPatch + 2 * m + 1] = textures[2 * m + 1];
                    textures[20 * numPointsPerPatch + 2 * m] = (float) (0.5
                            + 0.5 * Math.atan2(-v0.getY(), v0.getX()) / Math.PI);
                    ...
                    if (k < 6) {
                        textures[40 * numPointsPerPatch + 2 * m + 1] = textures[2 * m + 1];
                        textures[40 * numPointsPerPatch + 2 * m] = (float)
                                (0.5 + 0.5 * Math.atan2(v0.getY(), -v0.getX()) / Math.PI);
                        // modify texture coordinates to remedy the texture holes
                        if (Math.abs(textures[40 * numPointsPerPatch + 2 * m]) > 1 - EPS)
                            textures[40 * numPointsPerPatch + 2 * m] = 0;
                        textures[52 * numPointsPerPatch + 2 * m + 1] = textures[2 * m + 1];
                    ...
                    for (ii = 0; ii < 4; ii++)
                        for (jj = 0; jj < 4; jj++) {
                            x += dweights[i][ii] * weights[j][jj] * cpdata[patches[k][ii][jj]][0];
                            y += dweights[i][ii] * weights[j][jj] * cpdata[patches[k][ii][jj]][1];
                            z += dweights[i][ii] * weights[j][jj] * cpdata[patches[k][ii][jj]][2];
                        }
                    ...
                    for (ii = 0; ii < 4; ii++)
                        for (jj = 0; jj < 4; jj++) {
                            x += weights[i][ii] * dweights[j][jj] * cpdata[patches[k][ii][jj]][0];
                            y += weights[i][ii] * dweights[j][jj] * cpdata[patches[k][ii][jj]][1];
                            z += weights[i][ii] * dweights[j][jj] * cpdata[patches[k][ii][jj]][2];
                        }
                    v2 = new Vec3(x, y, z); // v-direction gradient
                    v0 = v1.cross(v2); // normal direction
                    v0.normalize();
                    normals[3 * m] = v0.getX();
        ...
        // each sub-patch (quad) is constructed by two triangles
        m = 0;
        for (k = 0; k < 10; k++) {
            for (i = 0; i < numPointsPerEdge - 1; i++)
                for (j = 0; j < numPointsPerEdge - 1; j++) {
                    indices[6 * m] = k * numPointsPerPatch + i * numPointsPerEdge + j;
                    indices[6 * m + 1] = indices[6 * m + 4] = k * numPointsPerPatch + i * numPointsPerEdge + j + 1;
                    indices[6 * m + 2] = indices[6 * m + 3] = k * numPointsPerPatch + (i + 1) * numPointsPerEdge + j;
                    indices[6 * m + 5] = k * numPointsPerPatch + (i + 1) * numPointsPerEdge + j + 1;
                    // y-axis reflection
                    n = 60 * numSubPatches;
                    indices[n + 6 * m] = 10 * numPointsPerPatch + indices[6 * m];
                    indices[n + 6 * m + 1] = indices[n + 6 * m + 4] = 10 * numPointsPerPatch + indices[6 * m + 2];
                    indices[n + 6 * m + 2] = indices[n + 6 * m + 3] = 10 * numPointsPerPatch + indices[6 * m + 1];
                    indices[n + 6 * m + 5] = 10 * numPointsPerPatch + indices[6 * m + 5];
                    // x and y-axis reflection
                    if (k < 6) {
                        n = 120 * numSubPatches;
                        indices[n + 6 * m] = 20 * numPointsPerPatch + indices[6 * m];
                        indices[n + 6 * m + 2] = indices[n + 6 * m + 3] = 20 * numPointsPerPatch + indices[6 * m + 1];
                        indices[n + 6 * m + 1] = indices[n + 6 * m + 4] = 20 * numPointsPerPatch + indices[6 * m + 2];
                        indices[n + 6 * m + 5] = 20 * numPointsPerPatch + indices[6 * m + 5];
                        n = 156 * numSubPatches;
                        indices[n + 6 * m] = 26 * numPointsPerPatch + indices[6 * m];
                        indices[n + 6 * m + 1] = indices[n + 6 * m + 4] = 26 * numPointsPerPatch + indices[6 * m + 1];
                        indices[n + 6 * m + 2] = indices[n + 6 * m + 3] = 26 * numPointsPerPatch + indices[6 * m + 2];
                        indices[n + 6 * m + 5] = 26 * numPointsPerPatch + indices[6 * m + 5];
                    }
                    m++;
                }
        }
