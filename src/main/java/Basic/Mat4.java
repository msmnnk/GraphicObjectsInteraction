...
public class Mat4 {
    ...
    // mv is supposed to be in a row-major order
    ...
    // of the matrix, or in a column-major order
    ...
    public void inverse() {
        ...
        if (t > EPS) { // back substitution to compute inverse matrix
            ...
            for (k = 0; k < 4; k++) {
                // find the biggest element in the sub-matrix
                r = k;
                c = k;
                t = Math.abs(temp[k][k]);
                for (i = k; i < 4; i++)
